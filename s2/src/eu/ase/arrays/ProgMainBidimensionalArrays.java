...
public class ProgMainBidimensionalArrays {
    public static void main(String[] args){
        ...
        short[][] studentsMarks = new short[][]{{5,5,9},{9,10,9}};//studentMarks.length=2
        //Linia 0 -> student 0 -> {5,5,9}
        //Linia 1 -> student 1 -> {9,10,9}

        float[] avgStudentMarks = new float[studNo];

        for(int i = 0; i < studNo; i++){
            avgStudentMarks[i]=0;

            for(int j = 0; j< lectNo; j ++){
                avgStudentMarks[i]=avgStudentMarks[i]+studentsMarks[i][j];
            }
            avgStudentMarks[i]/=lectNo;
        }
